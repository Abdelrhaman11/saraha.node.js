...
// const JWT_SECRET="A7laHambozoooooooooooooooo";


export const signUp=asyunkcHandler(async(req,res,next)=>{
...
export const confirmEmaill=asyunkcHandler(async(req,res,next)=>{
    ...
    // return user? res.redirect("http://localhost:4200/#/login"):res.send("<a>Ops looks like u don't have account yet follow me to signup now.</a>")

})



export const newConfirmEmail=asyunkcHandler(async(req,res,next)=>{
    ...
    if(!user)
    {
        ...
        // res.send("<a href="http://signup">Ops looks like u don't have account yet follow me to signup now.</a>")
    }
...
export const signIn=asyunkcHandler(async(req,res,next)=>{
        ...
        const token=JsonWebToken.sign({userName:user.userName , id:user._id , isOnline:true},process.env.TOKEN_SIGNATURE,{expiresIn:60*60})
